"""Integration tests for Confluent Kafka + Litestar."""
...
class TestConfluentLitestar(LitestarTestcase):
...
async def test_http_handler_receives_broker_via_di() -> None:
    """Confluent broker registered under explicit name='confluent'."""
    broker = KafkaBroker(KAFKA_URL)

    @subscriber("di-target-confluent")
...
async def test_after_startup_hook_runs_after_broker_start() -> None:
    broker = KafkaBroker(KAFKA_URL)
    cfg = BrokerConfig(broker=broker, name="confluent")
    order: list[str] = []
