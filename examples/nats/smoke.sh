#!/usr/bin/env bash
# Live-broker smoke for the FastStream Litestar example (NATS).
# Requires: docker, uv. Will start/stop a NATS container.
set -euo pipefail
...
if [[ "${SKIP_COMPOSE:-0}" != "1" ]]; then
  ...
  echo "==> SKIP_COMPOSE=1, using external nats"
fi
...
cd "$repo"
uv run litestar --app examples.nats.app:app run --port 8765 &
SERVER_PID=$!
