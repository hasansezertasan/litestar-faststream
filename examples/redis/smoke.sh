#!/usr/bin/env bash
# Live-broker smoke for the FastStream Litestar example (Redis).
# Requires: docker, uv. Will start/stop a Redis container.
set -euo pipefail
...
if [[ "${SKIP_COMPOSE:-0}" != "1" ]]; then
  ...
  echo "==> SKIP_COMPOSE=1, using external redis"
fi
...
cd "$repo"
uv run litestar --app examples.redis.app:app run --port 8765 &
SERVER_PID=$!
