#!/usr/bin/env bash
# Live-broker smoke for the FastStream Litestar example (MQTT / Mosquitto).
# Requires: docker, uv. Will start/stop a Mosquitto container.
set -euo pipefail
...
if [[ "${SKIP_COMPOSE:-0}" != "1" ]]; then
  ...
  echo "==> SKIP_COMPOSE=1, using external mosquitto"
fi
...
cd "$repo"
uv run litestar --app examples.mqtt.app:app run --port 8765 &
SERVER_PID=$!
